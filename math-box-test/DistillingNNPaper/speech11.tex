...
\[\recorddisplay{\btheta = \arg\max_{\btheta'} P(h_t | \bs_t;\btheta')}\]
where \recordmath{\btheta} are the parameters of our acoustic model \recordmath{P}
which maps acoustic observations at time \recordmath{t}, \recordmath{\bs_t}, to a probability, \recordmath{P(h_t | \bs_t;\btheta')} , of the ``correct''
HMM state \recordmath{h_t}, which is determined by a forced alignment with the correct sequence of words. The model is trained with
...
14,000 labels (HMM targets \recordmath{h_t}).  The input is 26 frames of 40 Mel-scaled filterbank coefficients with a 10ms advance
per frame and we predict the HMM state of 21\recordmath{^{st}} frame.  The total number of parameters is about 85M. This is a
...
We trained 10 separate models to predict \recordmath{P(h_t | \bs_t;\btheta)}, using exactly the same architecture and training
...
approach. For the distillation we tried temperatures of \recordmath{[1, {\bf 2}, 5, 10]} and used a relative weight of 0.5 on the
