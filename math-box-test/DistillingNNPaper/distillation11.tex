...
\recordmath{z_i}, computed for each class into a probability, \recordmath{q_i}, by comparing \recordmath{z_i} with the other logits.
\begin{equation}
\recorddisplay{q_i = \frac{exp(z_i/T)}{\sum_j exp(z_j/T)}}
\end{equation}
where \recordmath{T} is a temperature that is normally set to \recordmath{1}.  Using a higher value for \recordmath{T} produces a softer probability
...
gradients produced by the soft targets scale as \recordmath{1/T^2} it is
important to multiply them by \recordmath{T^2} when using both hard and soft
...
Each case in the transfer set contributes a cross-entropy gradient, \recordmath{dC/dz_i}, with respect to each logit, \recordmath{z_i} of the
distilled model.  If the cumbersome model has logits \recordmath{v_i} which produce soft target probabilities \recordmath{p_i} and the transfer training is done at a temperature of \recordmath{T},
this gradient is given by:
\begin{equation}
\recorddisplay{\frac{\partial C}{\partial z_i} = \frac{1}{T}\left(q_i - p_i\right) = \frac{1}{T}\left(\frac{e^{z_i/T}}{\sum_j e^{z_j/T}} -\frac{e^{v_i/T}}{\sum_j e^{v_j/T}}\right)}
\label{deriv} 
\end{equation}

If the temperature is high compared with the magnitude of the logits, we can approximate:
\begin{equation}
\recorddisplay{\frac{\partial C}{\partial z_i} \approx \frac{1}{T}\left( \frac{1+z_i/T}{N + \sum_j z_j/T} - \frac{1+v_i/T}{N + \sum_j v_j/T} \right)}
\label{mess}
\end{equation}

If we now assume that the logits have been zero-meaned separately for each transfer case so that \recordmath{\sum_j z_j = \sum_j v_j = 0} Eq. \ref{mess} simplifies to:
\begin{equation}
\recorddisplay{\frac{\partial C}{\partial z_i} \approx \frac{1}{NT^2}\left( z_i - v_i \right)}
\label{nice}
\end{equation}
So in the high temperature limit, distillation is equivalent to
minimizing \recordmath{{1/2}(z_i-v_i)^2}, provided the logits are zero-meaned
