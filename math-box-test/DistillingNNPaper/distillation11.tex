...
\recordmath{z_i}, computed for each class into a probability, \recordmath{q_i}, by comparing $z_i$ with the other logits.
\begin{equation}
\recorddisplay{q_i = \frac{exp(z_i/T)}{\sum_j exp(z_j/T)}}
\end{equation}
where \recordmath{T} is a temperature that is normally set to \recordmath{1}.  Using a higher value for $T$ produces a softer probability
