...
a set of classes \recordmath{S^m} that are often predicted together will be used as targets for one of our specialist models, \recordmath{m}. We applied an on-line
...
use. Given an input image \recordmath{\bx}, we do top-one classification in two steps:

Step 1: For each test case, we find
  the \recordmath{n} most probable classes according to the generalist model. Call this set of classes \recordmath{k}. In our
  experiments, we used \recordmath{n=1}.

Step 2: We then take all the specialist models, \recordmath{m}, whose special
subset of confusable classes,
  \recordmath{S^m}, has a non-empty intersection with \recordmath{k} and call this the active
  set of specialists \recordmath{A_k} (note that this set may be empty). We then
  find the full probability distribution \recordmath{\bq} over all the classes
  that minimizes:
\begin{equation}
\recorddisplay{KL (\bp^g, \bq) + \sum_{m \in A_k} KL (\bp^m , \bq)}
\label{eq:kl}
\end{equation}
where \recordmath{KL} denotes the KL divergence, and \recordmath{\bp^m} \recordmath{\bp^g} denote the
probability distribution of a specialist model or the generalist full
model. The distribution \recordmath{\bp^m} is a distribution over all the
specialist classes of \recordmath{m} plus a single dustbin class, so when
computing its KL divergence from the full \recordmath{\bq} distribution we sum
all of the probabilities that the full \recordmath{\bq} distribution assigns to
all the classes in \recordmath{m}'s dustbin.


Eq.~\ref{eq:kl} does not have a general closed form solution, though when all the models produce a single probability for
each class the solution is either the arithmetic or geometric mean, depending on whether we use \recordmath{KL(\bp,\bq)}
or \recordmath{KL(\bq,\bp)}). We parameterize \recordmath{\bq =
softmax(\bz)} (with \recordmath{T=1}) and we use gradient descent to optimize the logits \recordmath{\bz} w.r.t. eq.~\ref{eq:kl}. Note that this
